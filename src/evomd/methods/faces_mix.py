...
from ..core.sequence_geometry import (
...
class FacesMix(GenMethod):
    ...
    def generate(self, seq1: Sequence, seq2: Sequence, verbose=False) -> str:
        ...
        logger.debug("FacesMix: Computation of aligned positions")
        ...
        if verbose:
            print(f"Replacing {replacing_face} face")
            print(f"{''.join(protoseq)} <- {remaining_face} face")
