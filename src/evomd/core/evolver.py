...
class Evolver:
            ...
            for sq in self.instructor.sequences[:self.instructor.population]:
                if self.instructor.check_validity:
                    ...
                    if not self.is_valid_sequence(sq):
                        ...
                        continue
                self.sequences.append(Sequence(sq, h_scale=self.instructor.hydrofobic_scale, av_hm=self.instructor.average_hm))
    ...
    def include_sequences(self):
        ...
        for candidate in self.to_include:
            ...
            if self.sequence_exists(candidate):
                ...
                candidate = Sequence(candidate, generation=self.generations, h_scale=self.instructor.hydrofobic_scale, av_hm=self.instructor.average_hm)
    ...
    def populate(self) -> None:
        ...
        if not self.started:
            ...
            while len(self.sequences) < self.instructor.population:
                ...
                self.sequences.append(Sequence(candidate, h_scale=self.instructor.hydrofobic_scale, av_hm=self.instructor.average_hm))
            return
        ...
        while len(self.sequences) < target:
            ...
            logger.info(f'New sequence: {candidate}')
            self.sequences.append(Sequence(candidate, generation=self.generations, h_scale=self.instructor.hydrofobic_scale, av_hm=self.instructor.average_hm))
    ...
    def read_previous(self):
        ...
        for seq in sequences:
            ...
            try:
                new_seq.av_hm = json_data['av_hm']
            except:
                logger.warning(f"Json file don't include av_hm value. Default is False.")
                new_seq.av_hm = False
            # set directories
        ...
        for seq in sequences:
            ...
            if self.instructor.check_validity:
                ...
                if not self.instructor.generator._passes_restrictions(seq):
                    continue
            # create Sequence object
            new_seq = Sequence(seq, h_scale=self.instructor.hydrofobic_scale, av_hm=self.instructor.average_hm)
            # set directories
            new_seq.has_directory = True
            new_seq.directory = os.path.join(self.instructor.cwd, self.instructor.evomd_directory, seq)
            new_seq.last_iter_dir = os.path.join(new_seq.directory, 'iter_1')
    ...
    def read_report(self, report_path: str):
        ...
        with open(report_path, newline='') as fh:
            ...
            for row in reader:
                ...
                new_seq = Sequence(raw_seq, generation=generation, h_scale=self.instructor.hydrofobic_scale, av_hm=self.instructor.average_hm)
    ...
    # session lifecycle: shared by the CLI (evo-md) and the notebook/library API ---
    def show_evolver(self):
        """
        Print a human-readable summary of this Evolver.

        Equivalent to evo-md.py's --show-evolver action.
        """
        print(self)

    def start(self, fast_cycle=False):
        """
        Run the evolutionary loop until a termination criterion is met or
        stop_evolver() has flagged this Evolver as non-runnable.

        Populates the initial generation (a no-op if already at the target
        population size), then repeats iterate() -> sort_sequences() ->
        populate() -> sequence_backup() -> check_termination() until
        self.runnable is False. Unless fast_cycle is set, a pickle checkpoint
        is saved after every meaningful state change, so the run can be
        resumed with restart() after an interruption.

        The in-memory Evolver is the single source of truth for the whole
        run: unlike earlier versions, this loop does not reload the pickle
        from disk on every generation. Equivalent to evo-md.py's --start
        action (see restart() for the --restart equivalent).

        Args:
            fast_cycle (bool): If True, skip intermediate pickle checkpoints
                and per-sequence JSON backups (faster, less crash-resistant).
        """
        self.fast_cycle = fast_cycle

        # Initial population for the first generation (no-op if already full).
        self.populate()
        self.started = True
        if not self.fast_cycle:
            self.save_pkl()
    ...
    def restart(self, fast_cycle=False):
        """
        Resume an interrupted evolution.

        Sets recover_enabled so the next iterate() call inspects each
        sequence's state and skips steps already completed for it (see
        Manager.recover_pending_sequences), then runs the same loop as
        start(). Use restart() after an interruption; use start() if the
        previous run finished its last iteration cleanly.

        Equivalent to evo-md.py's --restart action.

        Args:
            fast_cycle (bool): forwarded to start().
        """
        self.recover_enabled = True
        self.start(fast_cycle=fast_cycle)

    def stop_evolver(self):
        """
        Mark this Evolver as intentionally stopped and persist that state.

        Sets runnable = False and saves a checkpoint, so that a later
        start()/restart() call on this object -- or `evo-md.py --start` /
        `--restart` run again on the same pickle -- does not resume this
        evolution by accident. This is a safety flag for managing several
        evolutions at once, not a live signal: it does not interrupt a
        start() loop that is currently running in this same process. Call it
        after a run has already stopped (e.g. after a KeyboardInterrupt), or
        from another process/terminal via `evo-md.py --stop-evolver`.

        Equivalent to evo-md.py's --stop-evolver action.
        """
        self.runnable = False
        self.save_pkl()
