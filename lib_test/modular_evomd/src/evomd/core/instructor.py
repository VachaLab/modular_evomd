...
def _is_yaml_file(instructions: str) -> bool:
    """
    Return True if 'instructions' is a path to an existing file.

    os.path.isfile() raises OSError on some platforms when given a string
    longer than the OS path-length limit -- which is exactly what happens
    when 'instructions' is raw multi-line YAML text rather than a path. That
    case is treated as "not a file" instead of propagating the error.
    """
    try:
        return os.path.isfile(instructions)
...
class Instructor:
    ...
    def __init__(self, instructions: str) -> None:
        """
        Build an Instructor from YAML instructions.

        For every option declared in `_schema`, the value is read from the YAML
        data; if it is absent (or None), the option's declared default is used.
        ...
        Args:
            instructions (str): Either a path to an existing YAML instruction
                ...
                built inline in a notebook). Both forms are read through the
                same io.StringIO-based path; which one was given is detected
                with os.path.isfile().
        """
        # self.filename is kept only for reference/logging: it is the source
        # path when 'instructions' was a real file, or None when it was raw
        # YAML text. Nothing else in the codebase depends on this attribute.
        self.filename = instructions if _is_yaml_file(instructions) else None
        self.yaml_data = self._load_yaml(instructions)
        ##############################
    ...
    # read yaml
    def _load_yaml(self, instructions: str) -> dict:
        """
        Load and return the YAML instructions as a dict.

        'instructions' may be a path to an existing file or raw YAML text.
        In both cases the content ends up read through an io.StringIO, so a
        file path and an equivalent pasted string behave identically.

        Args:
            instructions (str): Path to a YAML file, or raw YAML text.

        Returns:
            dict: Parsed YAML content (empty dict if it was empty).
        """
        if _is_yaml_file(instructions):
            with open(instructions, 'r', encoding='utf-8') as f:
                text = f.read()
        else:
            text = instructions
        return yaml.load(io.StringIO(text), Loader=yaml.FullLoader) or {}
    ...
    def _build_single_method(self, met):
        ...
        """
        if met == 'hybrids':
            from ..methods.hybrid import Hybrid
            return Hybrid()
        if met == 'swap':
            from ..methods.swap import Swap
            return Swap()
        if met == 'faces':
            from ..methods.faces_mix import FacesMix
            return FacesMix(slice_angle=self.face_slice_angle)
        if met == 'group_mutation':
            from ..methods.directed_mutations import GroupMutation
            return GroupMutation()
        if met == 'hydrophobic_mutation':
            from ..methods.directed_mutations import HydrophobicityMutation
    ...
    def set_gen_methods(self):
        ...
        if 'flanks' in self.populate_method:
            from ..methods.flanks import Flanks
