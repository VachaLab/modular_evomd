...
def get_arguments() -> argparse.Namespace:
    ...
    # --- run control (actions) -----------------------------------------------
    # Safety flag: marks the Evolver as non-runnable and saves, so a later
    # --start/--restart on this same pkl does not resume it by accident. Run
    # this after already stopping the process (e.g. Ctrl+C) -- it does not
    # signal a currently running --start loop in another process.
    parser.add_argument(
        '-stop', '--stop-evolver',
        action='store_true',
        help="Mark the Evolver as intentionally stopped, so a later --start "
             "or --restart on this pkl does not resume it by accident. Run "
             "this after the process has already stopped (e.g. Ctrl+C); it "
             "does not interrupt a currently running --start."
    )
